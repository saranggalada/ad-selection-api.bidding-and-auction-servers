...
resource "azurerm_kubernetes_cluster" "aks" {
  ...
  default_node_pool {
    ...
    min_count            = var.node_pool_settings.node_count
    max_count            = var.node_pool_settings.node_count * 2
...
resource "azurerm_role_assignment" "vn2_identity_rg_contributor" {
  principal_id                     = azurerm_kubernetes_cluster.aks.identity[0].principal_id
  role_definition_name             = "Contributor"
  scope                            = var.vn2_resource_group_id
  skip_service_principal_aad_check = true
...
resource "azurerm_role_assignment" "aks_kubeidentity_vn2_rg_contributor" {
  principal_id                     = azurerm_kubernetes_cluster.aks.kubelet_identity[0].object_id
  role_definition_name             = "Contributor"
  scope                            = var.vn2_resource_group_id
  skip_service_principal_aad_check = true
...
resource "azurerm_role_assignment" "aks_kubeidentity_vn2_mcrg_contributor" {
  principal_id                     = azurerm_kubernetes_cluster.aks.kubelet_identity[0].object_id
  role_definition_name             = "Contributor"
  scope                            = var.vn2_resource_group_id
  skip_service_principal_aad_check = true

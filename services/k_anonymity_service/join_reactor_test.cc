...
using RawRequest = JoinRequest::JoinRawRequest;
...
class JoinReactorTest : public testing::Test {
 protected:
  void SetUp() override {
    // initialize
    ...
    MetricContextMap<JoinRequest>(std::move(dependent_config))->Get(&request_);
  ...
  void CheckJoin(const RawRequest& raw_request,
    ...
    EXPECT_TRUE(diff.Compare(expected_response, response)) << diff_output;
  }
...
TEST_F(JoinReactorTest, JoinProducesResponse) {
  ...
  CheckJoin(raw_request, query_response);
}

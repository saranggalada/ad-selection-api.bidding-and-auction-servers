...
using microsoft::routing_utils::RoutingUtils;
...
#if defined(MICROSOFT_AD_SELECTION_BUILD)
ABSL_FLAG(std::optional<std::string>, selection_kv_server_addr, "",
          "Selection server Address to use for ads metadata lookup");
ABSL_FLAG(std::optional<int>, selection_kv_server_timeout_ms, std::nullopt,
          "The time in milliseconds to wait for the kv selection service to "
          "response");
ABSL_FLAG(std::optional<bool>, selection_kv_server_egress_tls, std::nullopt,
          "If true, selection service gRPC client uses TLS.");
#endif  // defined(MICROSOFT_AD_SELECTION_BUILD)
...
#if defined(MICROSOFT_AD_SELECTION_BUILD)
  config_client.SetFlag(FLAGS_selection_kv_server_addr,
                        microsoft::routing_utils::SELECTION_KV_SERVER_ADDR);
  config_client.SetFlag(
      FLAGS_selection_kv_server_egress_tls,
      microsoft::routing_utils::SELECTION_KV_SERVER_EGRESS_TLS);
  config_client.SetFlag(
      FLAGS_selection_kv_server_timeout_ms,
      microsoft::routing_utils::SELECTION_KV_SERVER_TIMEOUT_MS);
#endif  // defined(MICROSOFT_AD_SELECTION_BUILD)
...
DispatchConfig GetV8DispatchConfig(
    ...
    RoutingUtils* routing_utils) {
  return [&config_client, &enable_inference, routing_utils]() {
#else
    const TrustedServersConfigClient& config_client, bool enable_inference) {
...
#if defined(MICROSOFT_AD_SELECTION_BUILD)
    // In JavaScript UDF function will be available predefined function with
    // name microsoft::routing_utils::kRoutingFunctionName Roma Service will
    // call this function to get additional signals C++ function
    // microsoft::routing_utils::RoutingUtils::FetchAdditionalSignals will be
    // called by Roma Service, when in JavaScript UDF function
    // microsoft::routing_utils::kRoutingFunctionName is called
    PS_LOG(INFO) << "Registering "
                 << microsoft::routing_utils::kRoutingFunctionName
                 << "API in Roma Service";

    auto fetch_additional_signals_function_object = std::make_unique<
        google::scp::roma::FunctionBindingObjectV2<RomaRequestSharedContext>>();

    fetch_additional_signals_function_object->function_name =
        microsoft::routing_utils::kRoutingFunctionName;

    fetch_additional_signals_function_object->function =
        [routing_utils](google::scp::roma::FunctionBindingPayload<
                        privacy_sandbox::bidding_auction_servers::
                            RomaRequestSharedContext>& wrapper) {
          routing_utils->FetchAdditionalSignals(wrapper);
        };

    config.RegisterFunctionBinding(
        std::move(fetch_additional_signals_function_object));

    PS_LOG(INFO) << "Function "
                 << microsoft::routing_utils::kRoutingFunctionName
                 << " successfully registered in Roma Service";
#endif  // defined(MICROSOFT_AD_SELECTION_BUILD)
...
absl::Status RunServer() {
  ...
  std::unique_ptr<RoutingUtils> routing_utils =
      std::make_unique<RoutingUtils>(config_client, key_fetcher_manager.get());
#endif  // defined(MICROSOFT_AD_SELECTION_BUILD)
...
#if defined(MICROSOFT_AD_SELECTION_BUILD)
    v8_dispatcher = std::make_unique<V8Dispatcher>(GetV8DispatchConfig(
        config_client, enable_inference, routing_utils.get()));
#else
...
#if defined(MICROSOFT_AD_SELECTION_BUILD)
  runtime_config.microsoft_selection_kv_server_egress_tls =
      config_client.GetBooleanParameter(
          microsoft::routing_utils::SELECTION_KV_SERVER_EGRESS_TLS);
  runtime_config.microsoft_selection_kv_server_timeout_ms =
      config_client.GetIntParameter(
          microsoft::routing_utils::SELECTION_KV_SERVER_TIMEOUT_MS);
  runtime_config.microsoft_selection_kv_server_addr =
      std::string(config_client.GetStringParameter(
          microsoft::routing_utils::SELECTION_KV_SERVER_ADDR));
#endif  // defined(MICROSOFT_AD_SELECTION_BUILD)

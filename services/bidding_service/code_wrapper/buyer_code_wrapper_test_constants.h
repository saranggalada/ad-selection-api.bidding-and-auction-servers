...
R"JS_CODE(    function generateBid(interest_group, auction_signals,buyer_signals,)JS_CODE"
#else
R"JS_CODE(    function generateBids(interest_groups, auction_signals,buyer_signals,)JS_CODE"
...
constexpr absl::string_view kExpectedGenerateBidCode_template =
    R"JS_CODE(
  const globalWasmHex = [];
...
#if !defined(MICROSOFT_AD_SELECTION_BUILD)
    R"JS_CODE(    async function generateBidEntryFunction(interest_group, auction_signals, buyer_signals, trusted_bidding_signals, device_signals, multiBidLimit, featureFlags){)JS_CODE"
#else
    R"JS_CODE(    async function generateBidsEntryFunction(interest_groups, auction_signals, buyer_signals, trusted_bidding_signals, device_signals, multiBidLimit, featureFlags){)JS_CODE"
#endif  // !defined(MICROSOFT_AD_SELECTION_BUILD)
    R"JS_CODE(
...
#else
    R"JS_CODE(      for (const key in device_signals) {
...
#if !defined(MICROSOFT_AD_SELECTION_BUILD)
R"JS_CODE(        let response = await generateBid(interest_group, auction_signals, buyer_signals, trusted_bidding_signals, device_signals, multiBidLimit);)JS_CODE"
#else
R"JS_CODE(        let response = await generateBids(interest_groups, auction_signals, buyer_signals, trusted_bidding_signals, device_signals, multiBidLimit);)JS_CODE"
#endif  // !defined(MICROSOFT_AD_SELECTION_BUILD)
    R"JS_CODE(
...
#if !defined(MICROSOFT_AD_SELECTION_BUILD)
    R"JS_CODE(    async function generateBidEntryFunction(ads, sellerAuctionSignals, buyerSignals, preprocessedDataForRetrieval, encodedOnDeviceSignals, encodingVersion, multiBidLimit, auctionMetadata, featureFlags){)JS_CODE"
#else
    R"JS_CODE(    async function generateBidsEntryFunction(ads, sellerAuctionSignals, buyerSignals, preprocessedDataForRetrieval, encodedOnDeviceSignals, encodingVersion, multiBidLimit, auctionMetadata, featureFlags){)JS_CODE"
#endif  // !defined(MICROSOFT_AD_SELECTION_BUILD)
    R"JS_CODE(
      if (featureFlags.enable_logging){
        console.log = function(...args) {
...
#if !defined(MICROSOFT_AD_SELECTION_BUILD)
R"JS_CODE(        let response = await generateBid(ads, sellerAuctionSignals, buyerSignals, preprocessedDataForRetrieval, encodedOnDeviceSignals, encodingVersion, multiBidLimit, auctionMetadata);)JS_CODE"
#else
R"JS_CODE(        let response = await generateBids(ads, sellerAuctionSignals, buyerSignals, preprocessedDataForRetrieval, encodedOnDeviceSignals, encodingVersion, multiBidLimit, auctionMetadata);)JS_CODE"
#endif  // !defined(MICROSOFT_AD_SELECTION_BUILD)
R"JS_CODE(
        let generate_bid_response = [];
        if (response instanceof Array) {

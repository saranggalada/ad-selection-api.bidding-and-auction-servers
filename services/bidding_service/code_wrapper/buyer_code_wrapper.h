...
inline constexpr absl::string_view kEntryFunction = R"JS_CODE(
    ...
    R"JS_CODE(    async function generateBidEntryFunction($0, featureFlags){)JS_CODE"
#else
    R"JS_CODE(    async function generateBidsEntryFunction($0, featureFlags){)JS_CODE"
...
#if !defined(MICROSOFT_AD_SELECTION_BUILD)
R"JS_CODE(        let response = await generateBid($0);)JS_CODE"
#else
R"JS_CODE(        let response = await generateBids($0);)JS_CODE"
#endif  // !defined(MICROSOFT_AD_SELECTION_BUILD)
    // clang-format on
    R"JS_CODE(

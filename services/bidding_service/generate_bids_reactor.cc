...
// here:
// https://github.com/privacysandbox/fledge-docs/blob/main/bidding_auction_services_api.md#generatebid
//
...
constexpr std::string_view kDefaultMultiIGBidRequestName =
    "multi_ig_bid_request";
// Builds a Dispatch Request for Multiple Interest Group.
DispatchRequest MicrosoftBuildMultiIGBidRequest(
    const std::vector<std::shared_ptr<std::string>>& base_input,
    absl::string_view version) {
  // Create the optional object for the multi IG bid request.
  DispatchRequest multi_ig_bid_request;

  multi_ig_bid_request.id = kDefaultMultiIGBidRequestName;
  multi_ig_bid_request.version_string = version;
  // Copy base input and amend with custom interest_group
  multi_ig_bid_request.input = base_input;
  // Assign the handler name for the multi IG bid request.
  multi_ig_bid_request.handler_name =
      kDispatchHandlerFunctionNameWithCodeWrapper;

  // Edit to the input fields for the multi IG bid request to comply with a
  // multi bid object
  multi_ig_bid_request.input[ArgIndex(GenerateBidArgs::kInterestGroup)] =
      std::make_shared<std::string>(std::move(R"([)"));
  multi_ig_bid_request
      .input[ArgIndex(GenerateBidArgs::kTrustedBiddingSignals)] =
      std::make_shared<std::string>(std::move(R"({)"));

  multi_ig_bid_request.input[ArgIndex(GenerateBidArgs::kDeviceSignals)] =
      std::make_shared<std::string>(std::move(R"({)"));

  return multi_ig_bid_request;
}

void MicrosoftPostMultiIGBidRequestPreparation(
    DispatchRequest& multi_ig_bid_request, RequestLogContext& log_context) {
  // Balance the multi IG bid request object with closing brackets.
  absl::StrAppend(
      &(*multi_ig_bid_request.input[ArgIndex(GenerateBidArgs::kInterestGroup)]),
      "]");
  absl::StrAppend(
  ...
  if (server_common::log::PS_VLOG_IS_ON(10)) {
    PS_VLOG(10, log_context) << "\nMultiIGBidRequest Input Args:";
    for (const auto& it : multi_ig_bid_request.input) {
      PS_VLOG(10, log_context) << *it;
    }
  }
}

// Builds a Dispatch Request for the ROMA Engine for a Multiple Interest Groups.
absl::Status MicrosoftPopulateMultiIGBidRequest(
    IGForBidding& interest_group, DispatchRequest& multi_ig_bid_request,
    const RawRequest& raw_request, const bool enable_buyer_debug_url_generation,
    RequestLogContext& log_context, const bool enable_adtech_code_logging,
    const bool is_first_ig_in_multi_ig) {
  absl::StrAppend(
      &(*multi_ig_bid_request
             .input[ArgIndex(GenerateBidArgs::kTrustedBiddingSignals)]),
      is_first_ig_in_multi_ig ? R"(")" : R"(, ")", interest_group.name(),
      R"(":)",
      *(std::make_shared<std::string>(
          std::move(*interest_group.mutable_trusted_bidding_signals()))));
  interest_group.clear_trusted_bidding_signals();
  ...
  if (interest_group.has_browser_signals_for_bidding() &&
    ...
    absl::StrAppend(
        &(*multi_ig_bid_request
               .input[ArgIndex(GenerateBidArgs::kDeviceSignals)]),
        MakeBrowserSignalsForScript(
            raw_request.publisher_name(), raw_request.seller(),
            raw_request.top_level_seller(),
            interest_group.browser_signals_for_bidding(),
            raw_request.data_version(), raw_request.multi_bid_limit()));
    ...
    PS_ASSIGN_OR_RETURN(
        std::string serialized_android_signals,
        ProtoToJson(interest_group.android_signals_for_bidding()));
    absl::StrAppend(&(*multi_ig_bid_request
                           .input[ArgIndex(GenerateBidArgs::kDeviceSignals)]),
                    (serialized_android_signals.empty())
                        ? kEmptyDeviceSignals
                        : serialized_android_signals);
  } else {
    absl::StrAppend(&(*multi_ig_bid_request
  ...
  multi_ig_bid_request.input[ArgIndex(GenerateBidArgs::kMultiBidLimit)] =
      std::make_shared<std::string>(
          absl::StrCat(raw_request.multi_bid_limit()));
  multi_ig_bid_request.input[ArgIndex(GenerateBidArgs::kFeatureFlags)] =
      std::make_shared<std::string>(
          GetFeatureFlagJson(enable_adtech_code_logging,
                             enable_buyer_debug_url_generation &&
                                 raw_request.enable_debug_reporting()));
  ...
  absl::StrAppend(
      &(*multi_ig_bid_request.input[ArgIndex(GenerateBidArgs::kInterestGroup)]),
      is_first_ig_in_multi_ig ? "" : ", ", serialized_ig.value());
...
void MicrosoftConvertSingleBidRequestIntoArray(
    DispatchRequest& generate_bid_request) {
  // Convert the relevant input fields for the Single IG bid request into
  // arrays
  *generate_bid_request
       .input[ArgIndex(GenerateBidArgs::kInterestGroup)] = absl::StrCat(
      R"([)",
      *generate_bid_request.input[ArgIndex(GenerateBidArgs::kInterestGroup)],
      "]");
  *generate_bid_request
       .input[ArgIndex(GenerateBidArgs::kTrustedBiddingSignals)] = absl::StrCat(
      R"({")", generate_bid_request.id, R"(":)",
      *generate_bid_request
           .input[ArgIndex(GenerateBidArgs::kTrustedBiddingSignals)],
      "}");
  *generate_bid_request
       .input[ArgIndex(GenerateBidArgs::kDeviceSignals)] = absl::StrCat(
      R"({")", generate_bid_request.id, R"(":)",
      *generate_bid_request.input[ArgIndex(GenerateBidArgs::kDeviceSignals)],
      "}");
}
...
void GenerateBidsReactor::Execute() {
...
#if defined(MICROSOFT_AD_SELECTION_BUILD)
  // Request to hold multiple bids request
  DispatchRequest multi_ig_bid_request;

  // Keep track of multi ig population status
  absl::Status multi_ig_population_result = absl::OkStatus();
  // Format flag for the multi ig bid request
  bool is_first_ig_in_multi_ig = true;
#endif  // defined(MICROSOFT_AD_SELECTION_BUILD)
...
#if defined(MICROSOFT_AD_SELECTION_BUILD)
    // Check if the interest group can be combined into a single request
    if (MicrosoftCanCombineIGsIntoSingleRequest(interest_groups.at(i))) {
      if (is_first_ig_in_multi_ig) {
        multi_ig_bid_request = MicrosoftBuildMultiIGBidRequest(
            base_input, protected_audience_generate_bid_version_);
      }
      auto multi_ig_bid_request_backup = multi_ig_bid_request;

      // Update the status of the multi IG population with the current IG
      multi_ig_population_result.Update(MicrosoftPopulateMultiIGBidRequest(
          interest_groups.at(i), multi_ig_bid_request, raw_request_,
          enable_buyer_debug_url_generation_, log_context_,
          enable_adtech_code_logging_, is_first_ig_in_multi_ig));
      is_first_ig_in_multi_ig = false;

      if (!multi_ig_population_result.ok()) {
        PS_LOG(ERROR, log_context_)
            << "Failed to populate multi IG bid request: "
            << multi_ig_population_result.ToString(
                   absl::StatusToStringMode::kWithEverything);
        // Revert back changed if population of the current IG failed
        multi_ig_bid_request = multi_ig_bid_request_backup;
      }

    } else {
#endif  // defined(MICROSOFT_AD_SELECTION_BUILD)
      absl::StatusOr<DispatchRequest> generate_bid_request =
          BuildGenerateBidRequest(interest_groups.at(i), raw_request_,
                                  base_input,
...
#if defined(MICROSOFT_AD_SELECTION_BUILD)
        MicrosoftConvertSingleBidRequestIntoArray(generate_bid_request.value());
#endif  // defined(MICROSOFT_AD_SELECTION_BUILD)
  ...
  if (multi_ig_bid_request.input.size() > 0) {
    MicrosoftPostMultiIGBidRequestPreparation(multi_ig_bid_request,
                                              log_context_);

    multi_ig_bid_request.metadata =
        RomaSharedContextWithMetric<google::protobuf::Message>(
            request_, roma_request_context_factory_.Create(), log_context_);
    dispatch_requests_.push_back(multi_ig_bid_request);
  }
#endif  // defined(MICROSOFT_AD_SELECTION_BUILD)
...
#if defined(MICROSOFT_AD_SELECTION_BUILD)
      // Check interest group name is set, skip bid and log error if not.
      // Interest group name being set is prerequisite for a successful bid.
      if (bid.interest_group_name().empty() &&
          interest_group_name == kDefaultMultiIGBidRequestName) {
        PS_LOG(ERROR, log_context_)
            << "Skipping bid with no interest group name on response with id "
...
#if !defined(MICROSOFT_AD_SELECTION_BUILD)
      // This is to avoid resetting the interest group name for multi IG bid
      // back to the default name used when the request is prepared.
...
// TODO(task.ms/53054532): This function should check on a condition in the
// interest_group and decide whether merging is permitted for said IG.
bool GenerateBidsReactor::MicrosoftCanCombineIGsIntoSingleRequest(
    const IGForBidding& interest_group) {
  return true;
}
#endif  // defined(MICROSOFT_AD_SELECTION_BUILD)

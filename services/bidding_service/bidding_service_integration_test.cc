...
constexpr absl::string_view Microsoft_js_generate_bids_template = R"JS_CODE(
    function generateBids(interest_groups, auction_signals, buyer_signals,
                         trusted_bidding_signals, device_signals) {
      //Return an array of objects
      const multiIGReturnObject = [];
      for (let i = 0; i < interest_groups.length; i++) {
        const singleBid = generateBid(interest_groups[i], auction_signals, buyer_signals,
                                      trusted_bidding_signals[interest_groups[i].name],
                                      device_signals[interest_groups[i].name]);
        singleBid.interest_group_name = interest_groups[i].name;
        multiIGReturnObject.push(singleBid);
      }
      return multiIGReturnObject;
    }
    )JS_CODE";

constexpr absl::string_view
    Microsoft_js_generate_bids_with_debug_urls_template = R"JS_CODE(
    function generateBids(interest_groups, auction_signals, buyer_signals,
                         trusted_bidding_signals, device_signals) {
      //Return an array of objects
      const multiIGReturnObject = [];
      const adAuctionLossMap = new Map();
      const adAuctionWinMap = new Map();
      for (let i = 0; i < interest_groups.length; i++) {
        const singleBid = generateBid(interest_groups[i], auction_signals, buyer_signals,
                                      trusted_bidding_signals[interest_groups[i].name],
                                      device_signals[interest_groups[i].name]);
        singleBid.interest_group_name = interest_groups[i].name;
        adAuctionLossMap.set(singleBid.render, $0);
        adAuctionWinMap.set(singleBid.render, $1);
        multiIGReturnObject.push(singleBid);
      }
      $2forDebuggingOnly.reportAdAuctionLoss(adAuctionLossMap);
      $2forDebuggingOnly.reportAdAuctionWin(adAuctionWinMap);
      return multiIGReturnObject;
    }
    )JS_CODE";

constexpr absl::string_view Microsoft_js_generate_bids_for_array_template =
    R"JS_CODE(
    function generateBids(interest_groups, auction_signals, buyer_signals,
                         trusted_bidding_signals, device_signals) {
      //Return an array of objects
      const multiIGReturnObject = generateBid(interest_groups[0], auction_signals, buyer_signals,
                                      trusted_bidding_signals, device_signals);

      multiIGReturnObject.forEach(function(singleBid) {
        singleBid.interest_group_name = interest_groups[0].name;
      });
      return multiIGReturnObject;
    }
    )JS_CODE";
...
#if defined(MICROSOFT_AD_SELECTION_BUILD)
std::string MicrosoftAugmentJSCode(absl::string_view adtech_js) {
  std::string local_js_code_template =
      absl::StrFormat(js_code_template, kAdRenderUrlPrefixForTest);
  std::string local_js_code_with_buyer_reporting_id_template = absl::StrFormat(
      js_code_with_reporting_ids_template, kAdRenderUrlPrefixForTest,
      "testBuyerReportingId", "testBuyerAndSellerReportingId",
      "testSelectedBuyerAndSellerReportingId");
  std::string local_js_code_requiring_user_bidding_signals_template =
      absl::StrFormat(js_code_requiring_user_bidding_signals_template,
                      kAdRenderUrlPrefixForTest);
  std::string local_js_code_requiring_parsed_user_bidding_signals_template =
      absl::StrFormat(js_code_requiring_parsed_user_bidding_signals_template,
                      kAdRenderUrlPrefixForTest);
  std::string local_js_code_requiring_trusted_bidding_signals_template =
      absl::StrFormat(js_code_requiring_trusted_bidding_signals_template,
                      kAdRenderUrlPrefixForTest);
  std::string local_js_code_requiring_trusted_bidding_signals_keys_template =
      absl::StrFormat(js_code_requiring_trusted_bidding_signals_keys_template,
                      kAdRenderUrlPrefixForTest);
  std::string local_js_code_with_debug_urls_template = absl::StrFormat(
      js_code_with_debug_urls_template, kAdRenderUrlPrefixForTest);
  std::string local_js_code_with_very_large_debug_urls = absl::StrFormat(
      js_code_with_very_large_debug_urls, kAdRenderUrlPrefixForTest);
  std::string local_js_code_with_global_this_debug_urls_template =
      absl::StrFormat(js_code_with_global_this_debug_urls_template,
                      kAdRenderUrlPrefixForTest);
  std::string local_js_code_throws_exception =
      std::string(js_code_throws_exception);
  std::string local_js_code_with_logs_template =
      absl::StrFormat(js_code_with_logs_template, kAdRenderUrlPrefixForTest);
  std::string local_kBuyerBaseCodeForPrivateAggregation = absl::StrFormat(
      kBuyerBaseCodeForPrivateAggregation, kAdRenderUrlPrefixForTest);
  std::string local_kBuyerBaseCodeWithSignalValueAndSignalBucket =
      absl::StrFormat(kBuyerBaseCodeWithSignalValueAndSignalBucket,
                      kAdRenderUrlPrefixForTest);
  std::string local_js_code_requiring_null_trusted_bidding_signals_template =
      absl::StrFormat(js_code_requiring_null_trusted_bidding_signals_template,
                      kAdRenderUrlPrefixForTest);

  if (adtech_js == local_js_code_template ||
      adtech_js == local_js_code_with_buyer_reporting_id_template ||
      adtech_js == local_js_code_requiring_user_bidding_signals_template ||
      adtech_js ==
          local_js_code_requiring_parsed_user_bidding_signals_template ||
      adtech_js == local_js_code_requiring_trusted_bidding_signals_template ||
      adtech_js ==
          local_js_code_requiring_trusted_bidding_signals_keys_template ||
      adtech_js == js_code_throws_exception ||
      adtech_js == js_code_throws_exception_with_debug_urls ||
      adtech_js == local_js_code_with_logs_template ||
      adtech_js == kJsCodeWithTopLevelSeller ||
      adtech_js == kJsCodeWithComponentBidNotAllowed ||
      adtech_js == js_code_runs_wasm_helper ||
      adtech_js == local_kBuyerBaseCodeForPrivateAggregation ||
      adtech_js == local_kBuyerBaseCodeWithSignalValueAndSignalBucket ||
      adtech_js ==
          local_js_code_requiring_null_trusted_bidding_signals_template)
    return absl::StrCat(adtech_js, Microsoft_js_generate_bids_template);

  if (adtech_js == local_js_code_with_debug_urls_template)
    return absl::StrCat(
        adtech_js,
        absl::Substitute(Microsoft_js_generate_bids_with_debug_urls_template,
                         R"("https://example-dsp.com/debugLoss")",
                         R"("https://example-dsp.com/debugWin")", ""));

  if (adtech_js == local_js_code_with_very_large_debug_urls)
    return absl::StrCat(
        adtech_js,
        absl::Substitute(Microsoft_js_generate_bids_with_debug_urls_template,
                         R"(generateRandomUrl(65538))",
                         R"(generateRandomUrl(65536))", ""));

  if (adtech_js == local_js_code_with_global_this_debug_urls_template)
    return absl::StrCat(
        adtech_js,
        absl::Substitute(Microsoft_js_generate_bids_with_debug_urls_template,
                         R"("https://example-dsp.com/debugLoss")",
  ...
  if (adtech_js == js_code_with_multiple_bids_template ||
      adtech_js == js_code_with_debug_url_map_template ||
      adtech_js == js_code_with_debug_url_missing_key_template)
    return absl::StrCat(adtech_js,
                        Microsoft_js_generate_bids_for_array_template);

  return std::string(adtech_js);
}
#endif
...
#if defined(MICROSOFT_AD_SELECTION_BUILD)
  // Generated JS code is slightly different for the Ad Selection API from
  // upstream Here we will take the expected test strings and augment them to
  // match our intended output.
  std::string microsoft_adtech_js = MicrosoftAugmentJSCode(adtech_js);
  std::cout << "Microsoft adtech js: " << microsoft_adtech_js << std::endl;
  wrapper_config.auction_specific_setup =
      R"JSON(for (const key in device_signals) {
        device_signals[key].wasmHelper = globalWasmHelper;
      })JSON";
  std::string wrapper_blob =
      GetBuyerWrappedCode(microsoft_adtech_js, wrapper_config);

  // Here you can add std::cout of wrapper_blob to see the generated code
...
TEST_F(GenerateBidsReactorIntegrationTest,
  ...
  GenerateBidsRequest::GenerateBidsRawRequest raw_request;
#if defined(MICROSOFT_AD_SELECTION_BUILD)
  // We return array of bids and in generateBidsEntryFunction there is a check
  // for multiBidLimit. So we need to set it to a value greater than 1.
...
#else
    // This change should also be reflected on the following lines to be
    // actually correct. However, since all contributions are the same we are
    // just changing this line for now. When a final decision is made on how we
    // will handle this, it should be fixed in the future. TASK:
    // https://task.ms/55606868
    EXPECT_EQ(contributions.size(), 4);
#endif  // !defined(MICROSOFT_AD_SELECTION_BUILD)
...
TEST_F(GenerateBidsReactorIntegrationTest,
  ...
  for (const auto& ad_with_bid : raw_response.bids()) {
    ...
    // will handle this, it should be fixed in the future. TASK:
    // https://task.ms/55606868
    EXPECT_EQ(contributions.size(), 5);
#endif  // !defined(MICROSOFT_AD_SELECTION_BUILD)
    EXPECT_EQ(contributions.at(0).event().event_type(),
              EventType::EVENT_TYPE_WIN);
...
#if defined(MICROSOFT_AD_SELECTION_BUILD)
  // There are 2 issues with this test, a 3rd was added after the new changes in
  // the tests:
  // 1. This looks like a mistake on Google's part. The multi_bid_limit is set
  // to 10 by default but the test is expecting 100 bids. This will lead to the
  // code return an empty response object (as violating the multi_bid size
  // returns an empty response) hence this edit until the issue is resolved.

  // 2. The test is generating huge amounts of data due to the way the win and
  // loss urls are generated. This is not a realistic scenario and may be the
  // reason for the test not to fail, but to crash. The maximum size for
  // desired_bid_count I could reach was 67. This needs further analysis and
  // discussion.

  // 3. After the new changes in the tests, the new limit for this test to run
  // is 42. I believe the problem comes from the fact this test is generating
  // the random urls twice everytime (in the generateBid and generateBids
  // functions). I tried to access the urls generated in the generateBid but
  // could not. We'll have to investigate this further.
  test_config.desired_bid_count = 41;
  test_config.multi_bid_limit = desired_bid_count;
#endif  // defined(MICROSOFT_AD_SELECTION_BUILD)

...
struct BiddingServiceRuntimeConfig {
  ...
  // Endpoint where the kv selection server is listening for selection requests.
  std::string microsoft_selection_kv_server_addr = "";
  // Time to wait for the kv selection request to complete.
  int microsoft_selection_kv_server_timeout_ms = 60000;
  // Whether GRPC client to kv selection server should use TLS.
  bool microsoft_selection_kv_server_egress_tls = true;

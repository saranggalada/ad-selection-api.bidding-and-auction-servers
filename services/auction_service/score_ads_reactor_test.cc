...
TEST_F(ScoreAdsReactorTest, MicrosoftEnsureKAnonOnScoreAdResponse) {
  server_common::log::SetGlobalPSVLogLevel(10);
  MockV8DispatchClient dispatcher;
  AdWithBidMetadata foo, bar;
  foo = BuildTestAdWithBidMetadata();
  bar = GetTestAdWithBidBar();
  bar.set_microsoft_is_k_anon(true);
  RawRequest raw_request =
      BuildRawRequest({foo, bar}, {.enable_debug_reporting = true});
  ...
  EXPECT_TRUE(scored_ad.microsoft_is_k_anon());
}

// Validate if the winner is not k-anon but a non-winning bid is, we get false
// back.
TEST_F(ScoreAdsReactorTest, MicrosoftEnsureKAnonNotOnScoreAdResponse) {
  server_common::log::SetGlobalPSVLogLevel(10);
  MockV8DispatchClient dispatcher;
  AdWithBidMetadata foo, bar;
  foo = BuildTestAdWithBidMetadata();
  foo.set_microsoft_is_k_anon(true);
  bar = GetTestAdWithBidBar();
  RawRequest raw_request =
      BuildRawRequest({foo, bar}, {.enable_debug_reporting = true});
  ...
  const auto& scored_ad = raw_response.ad_score();
  PS_VLOG(5) << "Response:\n" << scored_ad.DebugString();
  EXPECT_FALSE(scored_ad.microsoft_is_k_anon());
}
#endif

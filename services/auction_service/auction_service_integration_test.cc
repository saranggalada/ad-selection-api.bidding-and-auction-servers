...
bool microsoft_g_create_k_anon = false;
#endif
...
#if defined(MICROSOFT_AD_SELECTION_BUILD)
    ad.set_microsoft_is_k_anon(microsoft_g_create_k_anon);
#endif
...
#if defined(MICROSOFT_AD_SELECTION_BUILD)
// Ensure k-anon input on a bid round trups and ends up on the response if the
// bid is a winner.
TEST_F(AuctionServiceIntegrationTest, MicrosoftKAnonRoundTripsTrue) {
  microsoft_g_create_k_anon = true;
  bool enable_debug_reporting = true;
  ScoreAdsRequest request =
      BuildScoreAdsRequest({.enable_debug_reporting = enable_debug_reporting});
  ...
  EXPECT_TRUE(scoredAd.microsoft_is_k_anon());
}

TEST_F(AuctionServiceIntegrationTest, MicrosoftKAnonRoundTripsFalse) {
  microsoft_g_create_k_anon = false;
  bool enable_debug_reporting = true;
  ScoreAdsRequest request =
      BuildScoreAdsRequest({.enable_debug_reporting = enable_debug_reporting});
  ...
  EXPECT_FALSE(scoredAd.microsoft_is_k_anon());
}
#endif

...
TYPED_TEST(SellerFrontEndServiceTest, MicrosoftRoundTripKAnon) {
  std::string decision_logic = "function scoreAds(){}";
  ...
  for (const auto& [buyer, buyerInput] : decoded_buyer_inputs) {
    ...
    auto get_bid_response = BuildGetBidsResponseWithSingleAd(
        buyer_to_ad_url.at(buyer),
        {.interest_group_name = buyerInput.interest_groups().Get(0).name()});

    get_bid_response.mutable_bids(0)->set_microsoft_is_k_anon(true);
    EXPECT_TRUE(get_bid_response.bids(0).microsoft_is_k_anon());
    EXPECT_FALSE(get_bid_response.bids(0).render().empty());
    EXPECT_EQ(get_bid_response.bids(0).ad_components_size(),
              kDefaultNumAdComponents);
    SetupBuyerClientMock(buyer, buyer_clients, get_bid_response);
  ...
  EXPECT_CALL(scoring_client, ExecuteInternal)
      ...
      .WillOnce([decision_logic, &scoring_done, &winner, this](
        ...
        // Last bid wins.
        for (const auto& bid : score_ads_request->ad_bids()) {
          // Every incoming bid should pass the k-anon value with it to the
          // score request.
          EXPECT_TRUE(bid.microsoft_is_k_anon());
          EXPECT_EQ(bid.ad_cost(), kAdCost);
          const std::string encoded_buyer_input =
              this->protected_auction_input_.buyer_input()
                  .find(bid.interest_group_owner())
          ...
          // The response, if setting k_anon should then transit back to the
          // SFE.
          score.set_microsoft_is_k_anon(true);
          *response.mutable_ad_score() = score;
          winner = score;
        }
        std::move(on_done)(
  ...
  EXPECT_EQ(auction_result.microsoft_is_winner_k_anon(),
            winner.microsoft_is_k_anon());
  EXPECT_TRUE(auction_result.microsoft_is_winner_k_anon());
  EXPECT_EQ(auction_result.score(), winner.desirability());
  EXPECT_EQ(auction_result.ad_component_render_urls_size(), 3);
  EXPECT_EQ(auction_result.ad_component_render_urls().size(),
            winner.mutable_component_renders()->size());
...
TYPED_TEST(SellerFrontEndServiceTest,
           MicrosoftAddNewInterestGroupToBidResponse) {
  std::string decision_logic = "function scoreAds(){}";
  ...
  for (const auto& [buyer, buyerInput] : decoded_buyer_inputs) {
    ...
    auto newAdWithBid = BuildNewAdWithBid(
        buyer_to_ad_url.at(buyer),
        {.interest_group_name = "new interest group", .bid_value = 1});

    get_bid_response.mutable_bids()->Add(std::move(newAdWithBid));
    EXPECT_EQ(get_bid_response.bids_size(), 2);
    EXPECT_FALSE(get_bid_response.bids(0).render().empty());
    EXPECT_EQ(get_bid_response.bids(0).ad_components_size(),
              kDefaultNumAdComponents);
    SetupBuyerClientMock(buyer, buyer_clients, get_bid_response);
  ...
  EXPECT_EQ(auction_result.interest_group_owner(),
            winner.interest_group_owner());
  for (auto& [buyer, expected_bid_response] : expected_buyer_bids) {
    EXPECT_EQ(expected_bid_response.get()->bids_size(), 2);
  }
}

...
#else
inline constexpr char kInvalidCborError[] =
    "Unable to parse AdSelectionInput CBOR";
#endif
...
#else
inline constexpr char kUnrecognizedProtectedAudienceKeyError[] =
    "Unrecognized key found while parsing AdSelectionInput: (%s)";
#endif
...
#if defined(MICROSOFT_AD_SELECTION_BUILD)
      case 8: {  // Microsoft Genuine Hint.
        bool is_valid_hint_type =
            IsTypeValid(&cbor_isa_string, entry.value, kMicrosoftGenuineHint,
                        kString, error_accumulator);
        RETURN_IF_PREV_ERRORS(error_accumulator, fail_fast, output);

        if (is_valid_hint_type) {
          output.set_microsoft_genuine_hint(CborDecodeString(entry.value));
        }
        break;
      }

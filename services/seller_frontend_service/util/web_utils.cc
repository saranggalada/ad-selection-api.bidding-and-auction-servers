...
absl::Status CborSerializeScoreAdResponse(
  ...
  PS_RETURN_IF_ERROR(CborSerializeBool(
      kMicrosoftIsKAnon, ad_score.microsoft_is_k_anon(), error_handler, root));
#endif
...
#if defined(MICROSOFT_AD_SELECTION_BUILD)
      case 21: {  // kMicrosoftIsKAnon
        if (!cbor_is_bool(kv.value)) {
          return absl::InvalidArgumentError(
              "Expected microsoft_winner_is_k_anon value to be a bool");
        }
        auction_result.set_microsoft_is_winner_k_anon(cbor_get_bool(kv.value));
      } break;
#endif
      default:

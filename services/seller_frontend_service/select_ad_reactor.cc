...
#if defined(MICROSOFT_AD_SELECTION_BUILD)
  result.set_microsoft_is_k_anon(input.microsoft_is_k_anon());
#endif
...
void SelectAdReactor::MicrosoftJoinWinnerKAnon(
  ...
  std::string genuine_hint;
  std::visit(
      [&genuine_hint](auto& input) {
        genuine_hint = input.microsoft_genuine_hint();
      },
      protected_auction_input_);

  if (genuine_hint.empty()) {
    // Not eligible for k-anon joining, return early.
    return;
  }

  const std::shared_ptr<BuyerFrontEndAsyncClient>& buyer_client =
      clients_.buyer_factory.Get(winning_ad.interest_group_owner());

  if (buyer_client == nullptr) {
    PS_VLOG(kNoisyWarn, log_context_) << "No buyer client found for buyer: "
                                      << winning_ad.interest_group_owner();
    return;
  }

  PS_VLOG(6, log_context_) << "Joining KAnon from a BFE";

  // Leverage the same timeouts for other SFE <> BFE communication to ensure
  // consistency.
  absl::Duration timeout = absl::Milliseconds(
      config_client_.GetIntParameter(GET_BID_RPC_TIMEOUT_MS));

  // Create request to join
  auto join_request = std::make_unique<
      MicrosoftJoinKAnonRequest::MicrosoftJoinKAnonRawRequest>();
  join_request->set_browser(genuine_hint);
  join_request->set_instance_hash(
      microsoft::k_anonymity::GetKAnonHash(winning_ad.render()));

  // gets deleted in execute internal callback
  auto bfe_request =
      metric::MakeInitiatedRequest(metric::kBfe, metric_context_.get())
          .release();
  bfe_request->SetBuyer(winning_ad.interest_group_owner());
  bfe_request->SetRequestSize((int)join_request->ByteSizeLong());

  grpc::ClientContext* client_context = client_contexts_.Add(buyer_metadata_);

  absl::Status execute_result =
      static_cast<BuyerFrontEndAsyncGrpcClient*>(buyer_client.get())
          ->MicrosoftExecuteJoin(
              std::move(join_request), client_context,
              CancellationWrapper(
                  request_context_, enable_cancellation_,
                  [this, bfe_request, buyer_client](
                    ...
                    PS_VLOG(6, log_context_)
                        << "KAnon Join completed: received a response from a "
                           "BFE";
                  },
                  [bfe_request]() {  // OnCancel
                    delete bfe_request;
  ...
  if (!execute_result.ok()) {
    LogIfError(
        metric_context_->AccumulateMetric<metric::kSfeErrorCountByErrorCode>(
            1, metric::kSfeGetBidsFailedToCall));
    PS_LOG(ERROR, log_context_) << absl::StrFormat(
        "Failed to make async KAnon Join call: (buyer: %s, "
        "seller: %s, error: "
        "%s)",
        winning_ad.interest_group_owner(), request_->auction_config().seller(),
...
#if defined(MICROSOFT_AD_SELECTION_BUILD)
      // We will perform a k-anon join call to the winning buyer/ad if we have
      // signal that this auction request is from a user and is genuine/unique.
      // In the future we will also call this for ghost winners to improve
      // aution performance.
      MicrosoftJoinWinnerKAnon(high_score.value());
#endif
    }
    if (found_response->has_debug_info()) {

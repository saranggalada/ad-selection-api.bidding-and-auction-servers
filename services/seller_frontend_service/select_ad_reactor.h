...
class SelectAdReactor : public grpc::ServerUnaryReactor {
 ...
 protected:
  ...
  // Helper to send a KAnon join request to the BFE.
  void MicrosoftJoinWinnerKAnon(ScoreAdsResponse::AdScore winning_ad);
#endif

  // Handles recording the fetched bid to state.
  // This is called by the grpc buyer client when the request is finished,
  // and will subsequently call update pending bids state which will update
  // how many bids are still pending and finally fetch the scoring signals
  // once all bids are done.
  //

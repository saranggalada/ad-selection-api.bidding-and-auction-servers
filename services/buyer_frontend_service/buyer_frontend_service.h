...
// Microsoft include section here
...
#if defined(MICROSOFT_AD_SELECTION_BUILD)
  // Clients to query the k-anonymity service run by the buyer for Query/Join
  // operations.
  std::unique_ptr<microsoft::k_anonymity::QueryAsyncClient>
      microsoft_kanon_query_async_client;
  std::unique_ptr<microsoft::k_anonymity::JoinAsyncClient>
...
#if defined(MICROSOFT_AD_SELECTION_BUILD)
  // Allows a SFE to indicate this BFE should consider a join to increment
  // k-anon counts given a winning ad.
  grpc::ServerUnaryReactor* MicrosoftJoinKAnon(
...
#if defined(MICROSOFT_AD_SELECTION_BUILD)
  // Stub to make GRPC calls to the KAnon service.
  std::unique_ptr<microsoft::k_anonymity::proto::KAnonymity::Stub>
      microsoft_k_anon_stub_;

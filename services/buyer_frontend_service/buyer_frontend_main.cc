...
ABSL_FLAG(std::optional<std::string>, k_anon_server_addr, std::nullopt,
          "K-Anonymity Server Address");
ABSL_FLAG(std::optional<int>, k_anon_server_timeout_ms, std::nullopt,
          "Max time to wait for fetching K-Anonymity result.");
#endif
...
#if defined(MICROSOFT_AD_SELECTION_BUILD)
  config_client.SetFlag(FLAGS_k_anon_server_addr, K_ANONYMITY_SERVER_ADDR);
  config_client.SetFlag(FLAGS_k_anon_server_timeout_ms,
...
#if defined(MICROSOFT_AD_SELECTION_BUILD)
  std::string k_anon_server_addr =
      std::string(config_client.GetStringParameter(K_ANONYMITY_SERVER_ADDR));
  int k_anon_server_timeout_ms =
      config_client.GetIntParameter(K_ANONYMITY_SERVER_TIMEOUT_MS);
#endif
...
#if defined(MICROSOFT_AD_SELECTION_BUILD)
          .microsoft_k_anon_server_addr = k_anon_server_addr,
#endif
...
#if defined(MICROSOFT_AD_SELECTION_BUILD)
          k_anon_server_timeout_ms,  // microsoft_k_anon_timeout_ms
#endif
          config_client.GetBooleanParameter(ENABLE_TKV_V2_BROWSER),

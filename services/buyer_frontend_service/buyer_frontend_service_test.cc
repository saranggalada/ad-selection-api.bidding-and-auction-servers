...
bool microsoft_expect_skipped_kanon = false;
std::unique_ptr<QueryAsyncClientMock> GetValidKAnonQueryClientMock() {
  std::unique_ptr<QueryAsyncClientMock> query_async_client =
      std::make_unique<QueryAsyncClientMock>();
  EXPECT_CALL(
      *query_async_client,
      ExecuteInternal(
          An<std::unique_ptr<
              microsoft::k_anonymity::proto::QueryRequest::QueryRawRequest>>(),
          An<grpc::ClientContext*>(),
          An<absl::AnyInvocable<void(absl::StatusOr<std::unique_ptr<
                                         microsoft::k_anonymity::proto::
                                             QueryResponse::QueryRawResponse>>,
                                     ResponseMetadata) &&>>(),
          An<absl::Duration>(), An<RequestConfig>()))
      .WillOnce([](std::unique_ptr<microsoft::k_anonymity::proto::QueryRequest::
                                       QueryRawRequest>
                       query_raw_request,
                   grpc::ClientContext* context, auto on_done,
                   absl::Duration timeout, RequestConfig request_config) {
        auto raw_response = std::make_unique<
            microsoft::k_anonymity::proto::QueryResponse::QueryRawResponse>();
        ABSL_LOG(INFO) << "K-Anon query request received.";

        auto instance_hashes = query_raw_request->instance_hash();
        for (const auto& query_hash : instance_hashes) {
          if (query_hash != "") {
            // For now we'll just say true to all requests!
            (*raw_response).add_is_k_anonymous(true);
          }
        }
        ABSL_LOG(INFO) << "K-Anon query request completed.";

        std::move(on_done)(std::move(raw_response),
                           /* response_metadata= */ {});

        ABSL_LOG(INFO) << "Callbacks completed.";
        return absl::OkStatus();
      });

  return query_async_client;
}

std::unique_ptr<QueryAsyncClientMock> GetNotCalledKAnonQueryClientMock() {
  std::unique_ptr<QueryAsyncClientMock> query_async_client =
      std::make_unique<QueryAsyncClientMock>();
  EXPECT_CALL(
      *query_async_client,
...
std::unique_ptr<JoinAsyncClientMock> GetValidKAnonJoinClientMock() {
  std::unique_ptr<JoinAsyncClientMock> join_async_client =
      std::make_unique<JoinAsyncClientMock>();
  EXPECT_CALL(*join_async_client, ExecuteInternal).Times(0);
  return join_async_client;
}
#endif
...
#if defined(MICROSOFT_AD_SELECTION_BUILD)
          .microsoft_kanon_query_async_client =
              microsoft_expect_skipped_kanon
                  ? GetNotCalledKAnonQueryClientMock()
                  : GetValidKAnonQueryClientMock(),
          .microsoft_kanon_join_async_client = GetValidKAnonJoinClientMock(),
#endif
...
#if defined(MICROSOFT_AD_SELECTION_BUILD)
      .microsoft_k_anon_timeout_ms = 60000,
#endif
  };
}
...
TEST_F(BuyerFrontEndServiceTest,
       ProtectedAudienceAndProtectedAppSignalsBidsFetchedHybrid) {
  auto bidding_signals_async_provider = SetupBiddingProviderMock(
      {.bidding_signals_value = valid_bidding_signals});
  std::unique_ptr<KVAsyncClientMock> kv_async_client =
      std::make_unique<KVAsyncClientMock>();
  ...
  ASSERT_TRUE(TextFormat::ParseFromString(
      absl::StrFormat(compression_group_wrapper,
                      absl::CEscape(RemoveWhiteSpaces(compression_group))),
      &response));
  SetupBiddingProviderMockHybrid(kv_async_client.get(), response,
...
TEST_F(BuyerFrontEndServiceTest,
       BiddingSignalsErrorAndProtectedAppSignalsBidsFetchedHybrid) {
  auto bidding_signals_async_provider = SetupBiddingProviderMock(
      {.bidding_signals_value = valid_bidding_signals});
  std::unique_ptr<KVAsyncClientMock> kv_async_client =
      GetBiddingSignalsProviderErrorMockV2();
...
TEST_F(BuyerFrontEndServiceTest,
       BiddingSignalsV1ErrorAndProtectedAppSignalsBidsFetchedHybrid) {
  auto bidding_signals_async_provider = GetBiddingSignalsProviderErrorMock();
  std::unique_ptr<KVAsyncClientMock> kv_async_client =
      std::make_unique<KVAsyncClientMock>();
  // v1/byos call fails ==> shouldn't be hitting v2/tkv
...
#if defined(MICROSOFT_AD_SELECTION_BUILD)
  // Do not expect call to k-anon as this is a test error scenario.
  microsoft_expect_skipped_kanon = true;
#endif
  BuyerFrontEndService buyer_frontend_service(
      CreateClientRegistry(
          std::move(bidding_signals_async_provider),
...
#if defined(MICROSOFT_AD_SELECTION_BUILD)
  // Return default value after previous test
  microsoft_expect_skipped_kanon = false;
#endif
  BuyerFrontEndService buyer_frontend_service(
      CreateClientRegistry(
          std::move(bidding_signals_async_provider),
...
#if defined(MICROSOFT_AD_SELECTION_BUILD)
  // We expect k-anon to be skipped as this validates a test error scenario.
  microsoft_expect_skipped_kanon = true;
#endif
  BuyerFrontEndService buyer_frontend_service(
      CreateClientRegistry(

 ...
 protected:
  void SetUp() override {
    ...
    EXPECT_CALL(
        query_client_mock_,
        ExecuteInternal(
            An<std::unique_ptr<microsoft::k_anonymity::proto::QueryRequest::
                                   QueryRawRequest>>(),
            An<grpc::ClientContext*>(),
            An<absl::AnyInvocable<
                void(absl::StatusOr<
                         std::unique_ptr<microsoft::k_anonymity::proto::
                                             QueryResponse::QueryRawResponse>>,
                     ResponseMetadata) &&>>(),
            An<absl::Duration>(), An<RequestConfig>()))
        .WillRepeatedly([](std::unique_ptr<microsoft::k_anonymity::proto::
                                               QueryRequest::QueryRawRequest>
                               query_raw_request,
...
#if defined(MICROSOFT_AD_SELECTION_BUILD)
  QueryAsyncClientMock query_client_mock_;
#endif
...
#if defined(MICROSOFT_AD_SELECTION_BUILD)
// Validate that all bids being returned to SFE have k-anon data in the bid
// response.
TEST_F(GetProtectedAppSignalsTest, MicrosoftAllBidsToSfeHaveKAnonData) {
  request_ = CreateGetBidsRequest(/*add_protected_signals_input=*/true,
                                  /*add_protected_audience_input=*/true);
  absl::BlockingCounter bids_counter(5);
  ...
  GetBidsResponse::GetBidsRawResponse raw_response;
  raw_response.ParseFromString(response_.response_ciphertext());
  ASSERT_EQ(raw_response.bids_size(), 2);
  ASSERT_EQ(raw_response.protected_app_signals_bids_size(), 3);

  for (const auto& ad : raw_response.bids()) {
    EXPECT_TRUE(ad.microsoft_is_k_anon());
  }

  for (const auto& ad : raw_response.protected_app_signals_bids()) {
    EXPECT_TRUE(ad.microsoft_is_k_anon());
  }
}
#endif

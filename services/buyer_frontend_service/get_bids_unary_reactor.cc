...
GetBidsUnaryReactor::GetBidsUnaryReactor(
    ...
    microsoft::k_anonymity::QueryAsyncClient&
...
#if defined(MICROSOFT_AD_SELECTION_BUILD)
      microsoft_kanon_query_async_client_(&microsoft_kanon_query_async_client),
#endif
...
#else
            // For the Ad Selection API, we will do some basic
            // context state checking then query for our
            // k-anon values. Once we get a response back we
            // will continue on with OnAllBidsDone.
            if (!any_successful_bid || context_->IsCancelled() ||
                !microsoft_kanon_query_async_client_) {
              PS_LOG(INFO, log_context_)
                  << "K-Anon Query: No successful bids or cancelled";
              OnAllBidsDone(any_successful_bid);
            } else {
              // No short circuits possible so just query k-anon
              const auto& log_context = raw_request_.log_context();
              auto k_anon_query_request =
                  microsoft::k_anonymity::CreateBidsQueryRawRequest(
                      *get_bids_raw_response_, log_context);
              grpc::ClientContext* client_context =
                  client_contexts_.Add(microsoft_k_anon_metadata_);
              absl::Status execute_result =
                  microsoft_kanon_query_async_client_->ExecuteInternal(
                      std::move(k_anon_query_request), client_context,
                      [this, &any_successful_bid](
                          absl::StatusOr<std::unique_ptr<
                              microsoft::k_anonymity::proto::QueryResponse::
                                  QueryRawResponse>>
                              raw_response,
                          ResponseMetadata response_metadata) {
                        microsoft::k_anonymity::AttachBiddingMetadata(
                            std::move(raw_response), *get_bids_raw_response_);

                        any_successful_bid = true;
                        OnAllBidsDone(any_successful_bid);
                      },
                      absl::Milliseconds(config_.microsoft_k_anon_timeout_ms));

              if (!execute_result.ok()) {
                PS_LOG(ERROR, log_context_) << "Danger Will Robinson!";
                LogIfError(metric_context_->AccumulateMetric<
                          metric::kBfeErrorCountByErrorCode>(
                    1,
                    metric::kBfeGenerateProtectedAppSignalsBidsFailedToCall));
                PS_LOG(ERROR, log_context_)
                    << "Failed to make async K-Anon Query call: "
                      "(error: "
...
GetBidsUnaryReactor::GetBidsUnaryReactor(
    ...
    : GetBidsUnaryReactor(context, get_bids_request, get_bids_response,
                          ...
                          microsoft_kanon_query_async_client,
#endif
                          bidding_async_client, config,
                          /*pas_bidding_async_client=*/nullptr,

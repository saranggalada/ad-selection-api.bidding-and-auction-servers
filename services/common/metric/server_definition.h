...
inline constexpr server_common::metrics::Definition<
    ...
    kBiddingZeroBidCount(
        ...
        "Total number of zero bids received in the bidding service", 15, 0);
...
inline constexpr server_common::metrics::Definition<
    ...
    kBiddingFailedToBidPercent(
        "bidding.business_logic.failed_to_bid_percent",
        "Percentage of interest groups per GenerateBidsRequest that failed to "
        "receive a bid (eg. due to dispatch error, UDF execution error, "
        "unparseable response from UDF, etc.)",
        kPercentHistogram, 1, 0);
...
inline constexpr server_common::metrics::Definition<
    double, server_common::metrics::Privacy::kNonImpacting,
    server_common::metrics::Instrument::kGauge>
    kInferenceModelDeletionSuccessCount(
        "inference.model_deletion.success_count",
        "Success count for deleting models from the inference sidecar");

inline constexpr server_common::metrics::Definition<
    double, server_common::metrics::Privacy::kNonImpacting,
    server_common::metrics::Instrument::kGauge>
    kInferenceModelDeletionFailedCountByStatus(
        "inference.model_deletion.failed_count_by_status",
        "Failed count for deleting models from the inference sidecar by "
        "status");
...
template <>
struct RequestMetric<MicrosoftJoinKAnonRequest> {
  static constexpr absl::Span<
      const server_common::metrics::DefinitionName* const>
      kList = kBfeMetricList;
  using ContextType = server_common::metrics::ServerContext<kList>;
};
inline auto* BfeJoinContextMap() {
  return MetricContextMap<MicrosoftJoinKAnonRequest>();
}
using BfeJoinContext = RequestMetric<MicrosoftJoinKAnonRequest>::ContextType;
#endif
...
#if defined(MICROSOFT_AD_SELECTION_BUILD)
// API to get `Context` for k-anon server to log metric
inline constexpr const server_common::metrics::DefinitionName*
    kKAnonymityMetricList[] = {
        &server_common::metrics::kTotalRequestCount,
        &server_common::metrics::kServerTotalTimeMs,
        &server_common::metrics::kRequestByte,
        &server_common::metrics::kResponseByte,
...
template <>
struct RequestMetric<microsoft::k_anonymity::proto::QueryRequest> {
  static constexpr absl::Span<
      const server_common::metrics::DefinitionName* const>
      kList = kKAnonymityMetricList;
  using ContextType = server_common::metrics::ServerContext<kList>;
};
inline auto* KAnonymityQueryContextMap() {
  return MetricContextMap<microsoft::k_anonymity::proto::QueryRequest>();
}
using KAnonymityQueryContext =
    RequestMetric<microsoft::k_anonymity::proto::QueryRequest>::ContextType;

template <>
struct RequestMetric<microsoft::k_anonymity::proto::JoinRequest> {
  static constexpr absl::Span<
      const server_common::metrics::DefinitionName* const>
      kList = kKAnonymityMetricList;
  using ContextType = server_common::metrics::ServerContext<kList>;
};
inline auto* KAnonymityJoinContextMap() {
  return MetricContextMap<microsoft::k_anonymity::proto::JoinRequest>();
}
using KAnonymityJoinContext =
    RequestMetric<microsoft::k_anonymity::proto::JoinRequest>::ContextType;
#endif  // MICROSOFT_AD_SELECTION_BUILD

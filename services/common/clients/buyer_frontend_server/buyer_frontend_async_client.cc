...
// Helper to execute a KAnon Join call where we don't care about the response.
absl::Status BuyerFrontEndAsyncGrpcClient::MicrosoftExecuteJoin(
    std::unique_ptr<MicrosoftJoinKAnonRequest::MicrosoftJoinKAnonRawRequest>
        raw_request,
    grpc::ClientContext* context,
    absl::AnyInvocable<
        void(absl::StatusOr<std::unique_ptr<google::protobuf::Empty>>,
             ResponseMetadata) &&>
        on_done,
    absl::Duration timeout) {
  PS_VLOG(5) << "BuyerFrontEndAsyncGrpcClient SendRpc KAnon Join invoked ...";

  PS_VLOG(6) << "Raw request:\n" << raw_request->DebugString();
  std::string plaintext = raw_request->SerializeAsString();
  PS_VLOG(6) << "Raw plain :\n" << plaintext;
  auto secret_request = EncryptRequestWithHpke<MicrosoftJoinKAnonRequest>(
  ...
  auto params =
      RawClientParams<MicrosoftJoinKAnonRequest, google::protobuf::Empty,
                      google::protobuf::Empty>(std::move(request),
                                               std::move(on_done));
  context->set_deadline(GetClientContextDeadline(timeout, kMaxClientTimeout));
  PS_VLOG(5) << "Sending RPC ...";

  stub_->async()->MicrosoftJoinKAnon(
